...
# Sparkline panel: Evolution of Large/Mid/Small cap mix over time (Streamlit + Plotly)
import streamlit as st
import pandas as pd
import numpy as np
import plotly.graph_objects as go
...
def load_marketcap_data(path=DATA_PATH):
    df = pd.read_excel(path)
    df.columns = [c.strip() if isinstance(c, str) else c for c in df.columns]
    # parse month_year or Date
    if "month_year" in df.columns:
        df["Date"] = pd.to_datetime(df["month_year"], errors="coerce").dt.to_period("M").dt.to_timestamp()
    else:
        date_cols = [c for c in df.columns if "date" in c.lower()]
        df["Date"] = pd.to_datetime(df[date_cols[0]], errors="coerce") if date_cols else pd.NaT
    # detect key columns
    col_map = {"fund": None, "cat": None, "contrib": None}
    for c in df.columns:
        lc = c.lower()
        if ("fund" in lc or "house" in lc or "scheme" in lc) and col_map["fund"] is None:
            col_map["fund"] = c
        if ("categor" in lc or "cap" in lc or "segment" in lc) and col_map["cat"] is None:
            col_map["cat"] = c
        if ("contrib" in lc or "contribution" in lc or "weight" in lc or "allocation" in lc) and col_map["contrib"] is None:
            col_map["contrib"] = c
    if None in col_map.values():
        raise ValueError(f"Could not detect required columns. Found: {df.columns.tolist()}")
    df = df.rename(columns={col_map["fund"]: "Fund House",
                            col_map["cat"]: "Categorization",
                            col_map["contrib"]: "Contribution"})
    # clean contribution
    df["Contribution"] = df["Contribution"].astype(str).str.replace("%", "", regex=False).str.replace(",", "", regex=False)
    df["Contribution"] = pd.to_numeric(df["Contribution"], errors="coerce")
    # if fractional (0-1) convert to percent
    if df["Contribution"].max() <= 1.01:
        df["Contribution"] = df["Contribution"] * 100
    # standardize categories
    df["Categorization"] = df["Categorization"].astype(str).str.strip().replace({
        "Large": "Large Cap", "LargeCap": "Large Cap", "Large-Cap": "Large Cap",
        "Mid": "Mid Cap", "MidCap": "Mid Cap", "Mid-Cap": "Mid Cap",
        "Small": "Small Cap", "SmallCap": "Small Cap", "Small-Cap": "Small Cap"
    })
...
# Load once (cached)
try:
    mc_df = load_marketcap_data()
except Exception as e:
    st.error(f"Error loading market-cap data: {e}")
    st.stop()

st.sidebar.markdown("### Sparklines: Market-cap mix over time")
all_funds = sorted(mc_df["Fund House"].unique())
# default: funds currently shown in main chart if available otherwise all
default_selected = all_funds if len(all_funds) <= 6 else all_funds[:6]
select_funds = st.sidebar.multiselect("Select funds for sparkline panel", options=all_funds, default=default_selected)

if not select_funds:
    st.info("Select at least one fund to view sparklines.")
else:
    # Aggregate into time series per fund & category
    ts = (
        mc_df[mc_df["Fund House"].isin(select_funds)]
        .groupby(["Fund House", "Date", "Categorization"], as_index=False)["Contribution"]
        .sum()
    )

    # Pivot so categories are columns
    ts_pivot = ts.pivot_table(index=["Fund House", "Date"], columns="Categorization", values="Contribution").reset_index()
    # Ensure columns for Large/Mid/Small exist
    for col in ["Large Cap", "Mid Cap", "Small Cap"]:
        if col not in ts_pivot.columns:
            ts_pivot[col] = 0.0

    # Sort dates
    ts_pivot = ts_pivot.sort_values(["Fund House", "Date"])

    st.subheader("Sparkline Panel — Market-cap Mix Over Time")
    st.markdown("Small sparklines show % allocation over time. Hover for exact values. Lines: Large (blue), Mid (green), Small (orange).")
    ...
    # color map consistent with earlier charts
    colors = {"Large Cap": "#577590", "Mid Cap": "#90be6d", "Small Cap": "#f4a259"}

    # Show legend only once (we will display a small legend above first row)
    show_legend = True
    ...
    for r in range(n_rows):
        ...
        for c in range(n_cols):
            if idx >= n:
                break
            fund = select_funds[idx]
            fund_df = ts_pivot[ts_pivot["Fund House"] == fund].sort_values("Date")
            dates = fund_df["Date"]
            fig = go.Figure()
            # add lines
            fig.add_trace(go.Scatter(x=dates, y=fund_df["Large Cap"], mode="lines+markers", name="Large Cap",
                                     line=dict(color=colors["Large Cap"], width=2), marker=dict(size=4)))
            fig.add_trace(go.Scatter(x=dates, y=fund_df["Mid Cap"], mode="lines+markers", name="Mid Cap",
                                     line=dict(color=colors["Mid Cap"], width=2), marker=dict(size=4)))
            fig.add_trace(go.Scatter(x=dates, y=fund_df["Small Cap"], mode="lines+markers", name="Small Cap",
                                     line=dict(color=colors["Small Cap"], width=2), marker=dict(size=4)))

            # layout minimal for sparkline
            fig.update_layout(
                title=dict(text=fund, x=0.01, xanchor="left", font=dict(size=12)),
            ...
            # show legend only on first sparkline to save space
            if show_legend:
                # keep legend displayed, then hide for next charts
                show_legend = False
            else:
    ...
    # Download the aggregated time series for selected funds
